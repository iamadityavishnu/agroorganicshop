...
if($cart_count != 0){
    ...
    while($row_products = mysqli_fetch_array($run_products)){
        ...
        $insert_customer_order = "insert into customer_orders (customer_id, product_id, product_title, product_thumbnail, due_amount, invoice_no, qty, weight, order_date, order_status)
    ...
    $address_in_link = "Total Amount (including shipping charges):%20" .($total_amount+49). "%0AAt%3A%0A%0A%2A" .$row_customer[1]. "%2A%0A" .$row_customer[7]. "%0A" .$row_customer[8]. "%0A" .$row_customer[5]. "%0A" .$row_customer[4]. "%0A" .$row_customer[9]. "%0A" .$row_customer[6]. "%0A" .$row_customer[2]. "%0A%0AInvoice%20number%3A" .$invoice_no;
